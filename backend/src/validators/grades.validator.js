...
const gradesValidators = {
  create: [
    ...
    body('grade_letter').optional({ values: 'null' }).isString().withMessage('Grade letter must be a string'),
    body('weight').optional({ values: 'null' }).isFloat({ min: 0, max: 1 }).withMessage('Weight must be between 0 and 1'),
    body('status').optional({ values: 'null' }).isIn(['DRAFT', 'PUBLISHED', 'LOCKED', 'UNLOCKED']).withMessage('Invalid status'),
  ],
  update: [
    param('id').isInt().withMessage('ID must be an integer'),
    body('grade_value').optional({ values: 'null' }).isFloat({ min: 0, max: 100 }).withMessage('Grade value must be between 0 and 100'),
    body('grade_letter').optional({ values: 'null' }).isString().withMessage('Grade letter must be a string'),
    body('weight').optional({ values: 'null' }).isFloat({ min: 0, max: 1 }).withMessage('Weight must be between 0 and 1'),
    body('status').optional({ values: 'null' }).isIn(['DRAFT', 'PUBLISHED', 'LOCKED', 'UNLOCKED']).withMessage('Invalid status'),
  ],
  ...
  findAll: [
    ...
    query('status').optional({ values: 'null' }).isIn(['DRAFT', 'PUBLISHED', 'LOCKED', 'UNLOCKED']).withMessage('Invalid status'),
