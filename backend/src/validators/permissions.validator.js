...
const permissionsValidators = {
  ...
  update: [
    ...
    body('module').optional({ values: 'null' }).isString().notEmpty().withMessage('Module must be a non-empty string'),
    body('action').optional({ values: 'null' }).isString().notEmpty().withMessage('Action must be a non-empty string'),
    body('description').optional({ values: 'null' }).isString().withMessage('Description must be a string'),
  ],
  findById: [
    param('id').isInt().withMessage('ID must be an integer'),
  ...
  findAll: [
    ...
    query('module').optional({ values: 'null' }).isString().withMessage('Module must be a string'),
    query('action').optional({ values: 'null' }).isString().withMessage('Action must be a string'),
