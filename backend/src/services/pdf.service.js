...
// FIX (bitácora 2026-09-15, pendiente "926 ms en /activity y /grades"): estas
// escrituras eran síncronas (`fs.mkdirSync`/`fs.writeFileSync`). Node es de un
// solo hilo: mientras escribe el PDF a disco, NINGUNA otra petición en curso
// avanza, aunque no tenga nada que ver con PDFs. Eso encaja con el síntoma del
// log — dos peticiones cualesquiera que estuvieran en vuelo en ese instante se
// congelan un rato mientras el resto, antes y después, corre normal. Se
// cambia a la versión asíncrona de fs/promises, que sí cede el hilo.
//
// Aviso honesto: esto reduce el bloqueo de la escritura a disco, pero el
// propio layout de pdfmake (diseño y paginación del PDF) también es
// CPU-bound y síncrono en Node — con un reporte grande puede seguir
// bloqueando unos milisegundos. Si el pico vuelve a aparecer con reportes
// largos, la solución de fondo es mover la generación del PDF a un
// worker_thread o a una cola de trabajos, no a este cambio puntual.
