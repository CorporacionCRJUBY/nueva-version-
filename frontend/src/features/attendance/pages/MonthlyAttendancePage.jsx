...
const STATUS_COLORS = {
  ...
  O: { bg: 'rgba(120,71,227,0.12)', text: '#6532c4' },
...
const MonthlyAttendancePage = () => {
  ...
  return (
    <Box style={sx({ p: 3 })}>
      ...
      {/* Info Card */}
      {gridData?.assignment && (
        <Paper style={sx({ p: 2, mb: 3, bgcolor: 'rgba(120,71,227,0.06)', borderLeft: '4px solid #7847e3' })}>
        ...
        <Paper style={sx({ overflow: 'hidden' })}>
          <TableContainer style={sx({ maxHeight: '65vh' })} className="scrollbar-thin">
            <Table stickyHeader size="small">
              <TableHead>
                <TableRow>
                  <TableCell className="sticky-col" style={sx({ minWidth: 160, fontWeight: 'bold', bgcolor: '#7847e3', color: '#FFF' })}>{t('attendance.student')}</TableCell>
                  <TableCell style={sx({ width: 60, fontWeight: 'bold', bgcolor: '#7847e3', color: '#FFF', textAlign: 'center' })}>{t('attendance.grade')}</TableCell>
                  {gridData.days?.map((d) => (
                    <TableCell
                      key={d.dayNumber}
                      style={sx({
                        ...
                        fontWeight: 'bold',
                        fontSize: '0.75rem',
                        bgcolor: d.isWeekend ? '#3d1c76' : '#7847e3',
                  ...
                  <TableCell style={sx({ width: 36, textAlign: 'center', fontWeight: 'bold', bgcolor: '#6532C4', color: '#FFF' })}>O</TableCell>
                  ...
                  <TableCell style={sx({ width: 60, textAlign: 'center', fontWeight: 'bold', bgcolor: '#7847e3', color: '#FFF' })}>{t('attendance.rate')}</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {gridData.students.length === 0 ? (
                  ...
                  gridData.students.map((st) => (
                    <TableRow key={st.id} hover>
                        ...
                        return (
                          <TableCell
                            ...
                            style={sx({
                              ...
                              color: colorInfo ? colorInfo.text : '#7d6f9c',
                              fontWeight: 'bold',
                              fontSize: '0.75rem',
                              '&:hover': {
                                bgcolor: d.isWeekend ? 'rgba(255,255,255,0.03)' : 'rgba(120,71,227,0.10)',
                              }
                            })}
                          >
                            {val || (d.isWeekend ? '·' : '-')}
                      ...
                      <TableCell style={sx({ textAlign: 'center', fontWeight: 700, color: '#6532c4' })}>
          ...
          {/* Legend */}
          <Box style={sx({ p: 2, display: 'flex', gap: 2, alignItems: 'center', flexWrap: 'wrap', bgcolor: '#faf9ff', borderTop: '1px solid #ebe4f7' })}>
            ...
            <Chip size="small" label={`O = ${t('attendance.online')}`} style={sx({ bgcolor: 'rgba(120,71,227,0.12)', color: '#6532c4', fontWeight: 'bold' })} />
