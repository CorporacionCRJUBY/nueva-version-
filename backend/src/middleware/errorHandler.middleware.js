...
function translateDatabaseError(err) {
  ...
  switch (code) {
    case 'ER_DUP_ENTRY': {
      // Mensajes claros para las restricciones únicas que el usuario se
      // encuentra a diario: el texto del motor («2-1-1-1-RC-1», el nombre del
      // índice) no le dice nada a quien está capturando datos.
      const FRIENDLY_UNIQUE_KEYS = {
        grade_records_unique_entry:
          'Ya existe una calificación para ese estudiante en esa materia, asignación y periodo. Edita la existente en vez de crear otra.',
        attendance_records_unique_entry:
          'Ya existe un registro de asistencia para ese estudiante en esa fecha y asignación.',
      };
      ...
      const friendly = key
        ? FRIENDLY_UNIQUE_KEYS[key] || FRIENDLY_UNIQUE_KEYS[key.split('.').pop()]
        : null;
