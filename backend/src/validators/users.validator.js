...
const profileUpdate = [
  body('email').optional().isEmail().withMessage('Invalid email address').isLength({ max: 254 }).withMessage('Email is too long'),
  body('full_name').optional().isString().trim().notEmpty().withMessage('Full name must be a non-empty string').isLength({ max: 100 }).withMessage('Full name is too long'),
  body('phone').optional({ nullable: true }).isString().withMessage('Phone must be a string').isLength({ max: 20 }).withMessage('Phone is too long'),
];
