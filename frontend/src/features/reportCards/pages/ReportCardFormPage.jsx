// FILE: frontend/src/features/reportCards/pages/ReportCardFormPage.jsx
...
const ReportCardFormPage = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const response = await reportCardsApi.getById(id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await reportCardsApi.update(id, formData);
      } else {
        await reportCardsApi.create(formData);
      }
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={isEdit ? t('reportCards.edit') : t('reportCards.add')}
        backTo="/report-cards"
      ...
      <form onSubmit={handleSubmit}>
        <FormGrid gap="loose">
          <FormCol size={{ xs: 12 }}>
            <FormSection title={t('reportCards.generalInfo')}><FormGrid gap="normal">
                <FormCol size={{ xs: 12, md: 6 }}>
                  <FormControl fullWidth required>
                    <InputLabel>{t('reportCards.student')}</InputLabel>
                    <Select name="student_id" value={formData.student_id || ''} onChange={handleChange} label={t('reportCards.student')}>
                      {students.map((s) => (
                ...
                <FormCol size={{ xs: 12, md: 6 }}>
                  <FormControl fullWidth required>
                    <InputLabel>{t('reportCards.period')}</InputLabel>
                    <Select name="academic_period_id" value={formData.academic_period_id || ''} onChange={handleChange} label={t('reportCards.period')}>
                ...
                <FormCol size={{ xs: 12, md: 6 }}>
                  <FormControl fullWidth required>
                    <InputLabel>{t('reportCards.year')}</InputLabel>
                    <Select name="academic_year_id" value={formData.academic_year_id || ''} onChange={handleChange} label={t('reportCards.year')}>
                      {academicYears.map((y) => (
                ...
                <FormCol size={{ xs: 12, md: 6 }}>
                  <FormControl fullWidth disabled={isEdit}>
                    <InputLabel>{t('reportCards.status')}</InputLabel>
                    <Select name="status" value={formData.status} onChange={handleChange} label={t('reportCards.status')}>
        ...
        <FormActions>
          ...
          <Button variant="outlined" startIcon={<CancelIcon />} onClick={() => navigate('/report-cards')}>
            {t('common.cancel')}
          </Button>
        </FormActions>
      </form>
      ...
      <Dialog open={confirmDeleteOpen} onClose={() => setConfirmDeleteOpen(false)}>
        ...
        <DialogContent>
          <DialogContentText>{t('reportCards.confirmDeleteReport')}</DialogContentText>
        </DialogContent>
        <DialogActions>
...
export default ReportCardFormPage;
