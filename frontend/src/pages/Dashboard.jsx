...
const DashboardPage = () => {
  ...
  const loadDashboardData = useCallback(async ({ silent = false } = {}) => {
    ...
    const [
      ...
      api.get('/students', { params: { pageSize: 200 } }),
      api.get('/teachers', { params: { pageSize: 1 } }),
      api.get('/subjects', { params: { pageSize: 100 } }),
      api.get('/grades', { params: { pageSize: 200 } }),
      api.get('/attendance', { params: { pageSize: 200 } }),
      api.get('/reports', { params: { pageSize: 1 } }),
      api.get('/activity', { params: { pageSize: 6 } }),
  ...
  const attendanceRate = useMemo(() => {
    ...
    const late = attendanceDistribution.find((d) => d.color === '#7847e3')?.value || 0;
    ...
    <div className="space-y-6">
      ...
      <header className="relative overflow-hidden rounded-xl bg-hero-gradient p-5 text-white shadow-lg sm:p-6">
        <div
          ...
          style={{
            backgroundImage:
              'radial-gradient(circle at 95% 0%, rgba(255,255,255,0.14) 0%, transparent 42%), radial-gradient(circle at 0% 100%, rgba(173,147,251,0.16) 0%, transparent 52%)',
          }}
        />
                        ...
                        <tr key={record.id || `${record.student_id}-${record.date}-${index}`} className="table-row">
                          ...
                          <td className="table-cell">
                            <span
                              className="chip"
                              style={{
                                backgroundColor: `${meta?.color || '#7d6f9c'}1a`,
                                color: meta?.color || '#7d6f9c',
                              }}
                            >
                              <span
                                className="badge-dot"
                                style={{ backgroundColor: meta?.color || '#7d6f9c' }}
                              />
