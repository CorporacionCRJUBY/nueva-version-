...
const Students = {
  ...
  // FIX (asignación de materias por el propio docente, 2026-09-16): un
  // "grupo" no es una tabla nueva — ya existe implícito en students como la
  // combinación grade+section+branch+año. Este resumen alimenta la pantalla
  // donde el docente busca sus grupos para autoasignarse una materia.
  groupsSummary(filters = {}) {
    const { branchId, branchIds, academicYearId } = filters;
    let query = db(TABLE).whereNull('deleted_at').where('status', 'ACTIVE');
    ...
    return query
      .select('grade', 'section', 'branch_id', 'academic_year_id')
      .count('id as student_count')
      .groupBy('grade', 'section', 'branch_id', 'academic_year_id')
      .orderBy('grade', 'asc')
      .orderBy('section', 'asc');
  },
