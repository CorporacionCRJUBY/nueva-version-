...
const StudentsService = {
  ...
  async getFullRecord(id, user) {
    ...
    // SEGURIDAD (2026-09-17): documentsRepository.findByStudent() trae la
    // fila cruda de `documents`, incluido `file_path` — una ruta ABSOLUTA
    // del servidor de archivos. documents.service.js ya sanea esto para sus
    // propios endpoints (findAll/findById), pero este expediente completo
    // del estudiante llamaba al repositorio directo y lo mandaba tal cual,
    // filtrando esa ruta interna a cualquiera que pudiera ver el expediente
    // (mismo hallazgo que ya se había corregido en documents.service.js).
    const sanitizedDocuments = documents.map(({ file_path, ...safeDoc }) => safeDoc);
